...
import { CCFISSurvey } from "./CCFISSurvey"

describe("CCFISSurvey", () => {
    let survey: CCFISSurvey
    const surveyName = "CCFIS"
    const surveySubtitle = "Cleveland Clinic Fecal Incontinence Score"
    const scaleName = "CCFIS"
    const scaleLength = 5
    const maxTotalScore = 20
    const optionScale = [0, 1, 2, 3, 4]

    beforeEach(() => {
        survey = new CCFISSurvey()
    })
    ...
    const testScaling = () => {
        ...
        it("should have scale question count of 5", () => {
    ...
    const testCalculateScore = () => {
        ...
        it("should calculate the percentage score of each scale", () => {
            ...
            // generate 5 questions with 5 options each
            const baseQuestions = generateBaseQuestions(
                scaleLength,
                optionScale,
                translate(`${scaleName}.scales`) as unknown as IScale[],
